    ...
    <div class="modal-content">
        ...
        <form id="districtForm" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="code" id="districtCode" class="form-input" required>
            </div>
            <div>
...
function loadStats() {
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                $('#totalRegions').text(response.total_regions || 0);
                $('#totalDistricts').text(response.total_districts || 0);
                $('#totalStations').text(response.total_stations || 0);
            }
        },
        error: function(xhr) {
...
function loadRegions(page = 1) {
    ...
    $('#regionsTableBody').html('<tr><td colspan="6" class="text-center py-8"><div class="loading-spinner"></div> Loading...</td></tr>');
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $('#regionsTableBody').html('<tr><td colspan="6" class="text-center py-8 text-red-500">Failed to load regions</td></tr>');
            }
        },
        error: function(xhr) {
            console.error('Regions error:', xhr);
            $('#regionsTableBody').html('<tr><td colspan="6" class="text-center py-8 text-red-500">Error loading regions</td></tr>');
        }
    });
}
...
function editRegion(id) {
    $.ajax({
        ...
        error: function() {
            Swal.fire('Error', 'Failed to load region data', 'error');
        }
    });
}
...
function loadDistricts(page = 1) {
    ...
    $('#districtsTableBody').html('<tr><td colspan="7" class="text-center py-8"><div class="loading-spinner"></div> Loading...</td></tr>');
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $('#districtsTableBody').html('<tr><td colspan="7" class="text-center py-8 text-red-500">Failed to load districts</td></tr>');
            }
        },
        error: function(xhr) {
            console.error('Districts error:', xhr);
            $('#districtsTableBody').html('<tr><td colspan="7" class="text-center py-8 text-red-500">Error loading districts</td></tr>');
        }
    });
}
    ...
    districts.forEach(district => {
        html += `
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4">${escapeHtml(district.region_name || 'N/A')}</td>
                <td class="px-6 py-4">
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                Swal.fire('Success', response.message, 'success');
                closeDistrictModal();
                loadDistricts(currentDistrictPage);
                loadStats();
            }
        },
        error: function(xhr) {
...
function loadStations(page = 1) {
    ...
    $('#stationsTableBody').html('<tr><td colspan="8" class="text-center py-8"><div class="loading-spinner"></div> Loading...</td></tr>');
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $('#stationsTableBody').html('<tr><td colspan="8" class="text-center py-8 text-red-500">Failed to load stations</td></tr>');
            }
        },
        error: function(xhr) {
            console.error('Stations error:', xhr);
            $('#stationsTableBody').html('<tr><td colspan="8" class="text-center py-8 text-red-500">Error loading stations</td></tr>');
        }
    });
}
    ...
    stations.forEach(station => {
        html += `
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4">${escapeHtml(station.region_name || 'N/A')}</td>
                <td class="px-6 py-4">${escapeHtml(station.district_name || 'N/A')}</td>
                <td class="px-6 py-4">
...
function openStationModal() {
    ...
    $('#stationRegionId').off('change').on('change', function() {
        ...
        if (regionId) {
            loadDistrictsForStation(regionId);
        } else {
...
function loadDistrictsForStation(regionId) {
    $.ajax({
        ...
        error: function() {
            $('#stationDistrictId').html('<option value="">Error loading districts</option>');
        }
    });
}
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                Swal.fire('Success', response.message, 'success');
                closeStationModal();
                loadStations(currentStationPage);
                loadStats();
            }
        },
        error: function(xhr) {
        ...
        success: function(response) {
            ...
            if (response.regions && response.regions.length) {
                response.regions.forEach(region => {
                    options += `<option value="${region.id}">${escapeHtml(region.name)}</option>`;
                });
            }
            $('#' + selectId).html(options);
        },
        error: function() {
            $('#' + selectId).html('<option value="">Error loading regions</option>');
        }
    });
}
...
function renderPagination(tab, pagination) {
    ...
    if (startPage > 1) {
        html += `<button onclick="load${tab.charAt(0).toUpperCase() + tab.slice(1)}(1)" class="px-3 py-1 border rounded hover:bg-gray-50">1</button>`;
    ...
    if (endPage < pagination.last_page) {
        if (endPage < pagination.last_page - 1) html += `<span class="px-2">...</span>`;
        html += `<button onclick="load${tab.charAt(0).toUpperCase() + tab.slice(1)}(${pagination.last_page})" class="px-3 py-1 border rounded hover:bg-gray-50">${pagination.last_page}</button>`;
    }
