...
<script>
    ...
    function fallbackCopyToClipboard(text, label = 'Content') {
        ...
        const notification = $(`
            <div class="fixed top-4 right-4 z-[9999] animate-slide-in">
                <div class="px-4 py-3 rounded-lg shadow-lg flex items-center gap-3 ${
                    ...
                    <button onclick="$(this).closest('.fixed').remove()" class="ml-4 text-white/80 hover:text-white" aria-label="Dismiss">
    ...
    // Global Slide-in animation for notifications
    $('<style>')
        .prop('type', 'text/css')
        .html(`
            @keyframes slideIn {
