...
class AiSupportService
{
    protected function getSystemPrompt(?User $user): string
    {
        ...
        You are currently assisting {$userName} (Role: {$role}).
        ...
        if ($role === 'driver') {
            $prompt .= " As a driver, focus on vehicle assignments, mileage logging, and maintenance requests.";
        } elseif ($role === 'admin' || $role === 'super_admin') {
            $prompt .= " As an administrator, focus on fleet-wide analytics, tracking all units, and managing reports.";
        }
